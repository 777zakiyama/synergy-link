...
import { User, MapPin, Target, Palette } from 'lucide-react'
...
function App() {
  ...
  const generateVisionMap = async () => {
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_URL}/users/${userId}/profile`, {
        method: 'POST',
      ...
      for (const connection of network) {
        await fetch(`${import.meta.env.VITE_API_URL}/users/${userId}/network`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/users/${userId}/vision-map?template=${selectedTemplate}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
